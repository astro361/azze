...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const triggerDeployment = useCallback(async (repo: GitHubRepo) => {
    ...
    try {
      ...
      // Update Supabase to explicitly track this real Vercel URL
      ...
      setProjects(prev => prev.map(p => 
        p.id === selectedProject.id ? { ...p, status: 'ACTIVE', deployedUrl: liveUrl } : p
      ));
      
      // Start log stream animation (Optional simulated stream UI wrapper for the terminal)
      streamBuildLogs(newDeployment.id);
    } catch (err: any) {
      console.error('Deployment Failed:', err);
      alert(err.message || 'Failed to trigger Vercel deployment.');
      
      // Mark as failed if we couldn't connect
