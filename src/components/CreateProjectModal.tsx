...
export function CreateProjectModal({ onClose }: CreateProjectModalProps) {
  const { createProject, isDashboardLoading, githubRepos } = useApp();
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 pointer-events-none" viewBox="0 0 24 24" fill="currentColor">
              ...
              {githubRepos && githubRepos.length > 0 ? (
                <select
                  ...
                  required
                >
                  <option value="" disabled hidden>Select a repository...</option>
                  {githubRepos.map(repo => (
                    <option key={repo.id} value={repo.clone_url || repo.html_url}>
                      {repo.full_name}
                    </option>
                  ))}
                </select>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button 
                    ...
                    className="text-sm text-[#c37a4c] font-medium hover:underline text-left mt-1"
                  >
                    Connect GitHub to auto-sync your repositories
                  </button>
                </div>
              )}
            </div>
