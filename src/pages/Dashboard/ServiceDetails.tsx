...
export function ServiceDetails() {
  ...
  const handleRedeploy = async () => {
    // Trigger redeploy for the specific repo linked to this service
    if (!service) return;
    try {
      // Simulate build status change
      ...
      // Simulate successful build after 5 seconds
      setTimeout(async () => {
        const liveUrl = `${window.location.origin}/deployments/${service.name.toLowerCase()}`;
